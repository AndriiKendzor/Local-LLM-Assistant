...
import ctypes
import threading
from db import save_conversation, load_chats, search_chat
...
def create_buttons(llm_response, user_text, chat_column):
    def toggle_heart(e):
...
# --- functions ---
def create_new_chat(chat_column, chat_name_container, add_knowledge_base):

    globals.context = ""
    globals.llm_model = globals.model_list[0] if globals.model_list else None
    globals.stop_response = False
    globals.knowlage_base_added = False
    globals.collection = None
    globals.dialog = None
    globals.title = "New chat"
    ...
    globals.chat_id = generate_random_code()
    print(globals.chat_id)

    # Ініціалізація LLM
    if globals.llm_model:
        globals.model = OllamaLLM(model=globals.llm_model)
        globals.prompt = ChatPromptTemplate.from_template(globals.template)
        globals.chain = globals.prompt | globals.model
    else:
        globals.model = None
        globals.prompt = None
        globals.chain = None
...
def build_ui(page):
    ...
    def adjust_height(e):
        if page.width >= history_container.width + 15 + 850:
            width = 800
        else:
            width = page.width - history_container.width - 15 - 100
    ...
    def create_massage(text):
        ...
        # Виведення всіх глобальних змінних
        print(f"context = {globals.context}")
        print(f"model_list = {globals.model_list}")
        print(f"llm_model = {globals.llm_model}")
        print(f"stop_response = {globals.stop_response}")
        print(f"knowlage_base_added = {globals.knowlage_base_added}")
        print(f"collection = {globals.collection}")
        print(f"dialog = {globals.dialog}")
        print(f"chat_id = {globals.chat_id}")
        print(f"model = {globals.model}")
        print(f"prompt = {globals.prompt}")
        print(f"chain = {globals.chain}")
        ...
        if text != "":
            ...
            # Викликаємо LLM
            try:
                llm_response = call_llm(text, False)
            ...
            buttons_row = create_buttons(printed_text, text, chat_column)
            ...
            save_conversation(llm_response, text, chat_name_container, chats_column, page, chat_column, model_chose_dropdown, add_knowledge_base)
    ...
    if not ollama_installed or not globals.model_list:
    ...
    def dropdown_changed(e):
        ...
        globals.llm_model = model_chose_dropdown.value
    ...
    model_chose_dropdown = ft.Dropdown(
        label="",
        value=globals.model_list[0],  # Вибір за замовчуванням
        options=[ft.dropdown.Option(model) for model in globals.model_list],
    ...
    def create_rag():
        ...
        def on_folder_picked(e: ft.FilePickerResultEvent):
            if e.path:
                ...
                globals.knowlage_base_added = True
                # show path
                ...
                globals.dialog.actions.clear()
                globals.dialog.actions.append(waiting_text)
                page.update()
                # Запускаємо аніміцію очікування у синхронному режимі
                stop_flag = {"stop": False}
                ...
                # Викликаємо LLM
                try:
                    scan_documents(globals.chat_id, directory_path[0])
                    new_style = ft.ButtonStyle(
                        padding=5,
                        alignment=ft.alignment.center,
                    ...
                    globals.dialog.actions.append(close_button)
                    globals.dialog.actions.append(chose_folder_button)
                    globals.dialog.actions.append(clean_button)
                except Exception as e:
        ...
        def scan_documents(chat_id, path):
            # create collection
            globals.collection = create_chromadb_collection(chat_id)
        ...
        if globals.dialog is None:
            # Створюємо діалогове вікно
            globals.dialog = ft.AlertDialog(
                title=ft.Text(
            ...
            page.dialog = globals.dialog

        # Відкриваємо діалогове вікно
        globals.dialog.open = True
        page.update()
    ...
    new_chat_button = ft.TextButton(
        ...
        on_click=lambda e: create_new_chat(chat_column, chat_name_container, add_knowledge_base)
    )
    search_chat_button = ft.TextButton(
        ...
        on_click=lambda e: search_chat(page)
    )
    ...
    chats_column = ft.Column(
        controls=load_chats(page, chat_column, chat_name_container, model_chose_dropdown, add_knowledge_base),
        alignment=ft.MainAxisAlignment.START,  # Вирівнювання елементів зверху
        horizontal_alignment=ft.CrossAxisAlignment.START,
        scroll=ft.ScrollMode.AUTO,
    )
    chats_container = ft.Container(
        ...
        border_radius=10,
        bgcolor="black",
        alignment=ft.alignment.top_left,  # Вирівнювання контенту у верхній лівий кут
    )
